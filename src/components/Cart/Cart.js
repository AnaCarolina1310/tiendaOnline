...
import "./Cart.css";
export const Cart = () => {
  const { cart, precioTotal, vaciarCarrito, eliminarProducto } =
    ...
    <div className="contenedor-card">
        ...
        <div key={ele.id} className="card-check">
          ...
          <div className="boton-eliminar-contenedor">
            <button
              ...
              onClick={() => eliminarProducto(ele.id)}
            >
              <BsTrash />
            </button>
            <hr />
      ...
      {cart.length > 0 ? (
        <div className="botones">
          <button className="btn btn-dark m-3" onClick={vaciarCarrito}>
            Vaciar carrito
          </button>
          <Link to="/checkout" className="btn btn-dark m-3">
            Finalizar compra
          </Link>
        </div>
      ) : (
